...
const StatsCards = () => {
  ...
  useEffect(() => {
    const fetchGroup = async () => {
      const res = await fetch(`http://localhost:5001/api/get-default-group?userid=${userid}`);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      events.forEach(event => {
        ...
        console.log(`🕒 ${event.name} created at ${createdAt}, comparing to ${oneHourAgo}`);
      
        if (!isNaN(createdTime) && createdTime > oneHourAgo.getTime()) {
          console.log(`🔥 Match: ${event.name} gained ${event.aura_points}`);
